...
/*
IComparable
    override ToCompare method. 
    It takes one argument, the object type. 
    So it compares the current object with the object next inline;
    e.g., to sort the list of integers sort method does bubble sort on elements.

    So as you can see ToCompare method returns an int. 
    So if the current value is bigger than the next value it returns 1, 
    else -1. 
    And if both of them are the same then it returns 0
...
namespace DesignPattern.CSharp
{
    ...
    public class SmartPhone : IComparable
    {
        ...
        {
            //obj is : next object which is coming as a paramete
